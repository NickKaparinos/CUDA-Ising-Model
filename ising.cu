...
__global__ void kernelCalcNewLattice(int* oldLattice, int* newLattice, double* w, int n,int dim){
    // Each thread will calculate the new magnetic moments of a block of the lattice.The top left cell of that block has coordinates (iStart,jStart).
	int iStart = blockIdx.x * dim;
	int jStart = threadIdx.x * dim;

    for(int i = iStart; i <= (iStart + (dim-1)) && ( i<n ); i++){
        for(int j = jStart; j <= (jStart + (dim-1)) && ( j<n ); j++){
...
void calcNewLattice(int* oldLattice, int* newLattice, double* w, int n, int iteration){
    ...
    int dim = 50;                               // Every thread will calculate the moments of a dim x dim block of the lattice
    int numOfBlocks = ceil((double)n/dim);      // Number of blocks in each dimension 

    kernelCalcNewLattice<<<numOfBlocks,numOfBlocks>>>(dev_oldLattice, dev_newLattice, dev_w, n, dim);
